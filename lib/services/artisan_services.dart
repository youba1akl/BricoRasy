  ...
  static Future<List<Artisan>> fetchArtisans() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Échec du chargement des artisans: ${response.statusCode}');
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  static Future<Artisan> fetchArtisanById(String userId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Échec du chargement du profil artisan (ID: $userId): ${response.statusCode}');
      }
    } catch (e) {
      if (kDebugMode) {
        print("ArtisanService: Network error fetching artisan by ID $userId - $e");
      }
    ...
    if (AuthService.currentUser != null) {
      if (AuthService.currentUser!.isArtisan) {
        ...
        return Artisan(
          ...
          job: AuthService.currentUser!.job ?? "Non spécifié", // Default if null
          localisation: AuthService.currentUser!.localisation ?? "Non spécifiée",
          numTel: AuthService.currentUser!.phone, // User's phone is the numTel for artisan
          rating: "N/A", // Rating usually comes from aggregated reviews, not directly on user model
          image: AuthService.currentUser!.profilePicture ?? '', // Use profilePicture or default
          like: "0", // Likes usually come from a separate system or aggregation
        );
      } else {
        if (kDebugMode) {
          print("ArtisanService: Logged-in user (${AuthService.currentUser!.fullname}) is not an artisan (role: ${AuthService.currentUser!.role}).");
        }
        return null; // Logged-in user is not an artisan
      }
    } else {
      if (kDebugMode) {
        print("ArtisanService: No user currently logged in (AuthService.currentUser is null).");
      }

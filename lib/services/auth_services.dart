...
import 'package:shared_preferences/shared_preferences.dart'; // Adjust path if necessary
...
class AuthService {
  static const String baseUrl =
      "http://192.168.1.33:5000"; // Your local IP for backend
  ...
  static void setCurrentUser(Map<String, dynamic> userData) {
    try {
      ...
      if (kDebugMode) {
        print(
          "AuthService: User set - ID: ${currentUser!.id}, Name: ${currentUser!.fullname}, Role: ${currentUser!.role}",
        );
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  static Future<bool> sendOtp(String email) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode)
          print(
            "AuthService: Error sending OTP to $email - ${response.statusCode}: ${response.body}",
          );
        return false;
      }
    } catch (e) {
  ...
  static Future<bool> verifyOtp(String email, String otp) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (kDebugMode)
          print("AuthService: OTP verified successfully for $email");
        return true;
      } else {
        if (kDebugMode)
          print(
            "AuthService: OTP verification failed for $email - ${response.statusCode}: ${response.body}",
          );
        return false;
      }
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> loginUser(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode)
          print(
            "AuthService: Login failed - ${response.statusCode}: $errorMessage",
          );
        clearCurrentUser();
      ...
      return {
        'success': false,
        'message': 'Erreur réseau lors de la connexion: $e',
      };
    }
  }
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'genre': genre,
          'services': services, // uniquement pour les artisans
          'posts': posts,
        }),
      );
      ...
      if (response.statusCode == 201) {
        ...
        setCurrentUser(
          data,
        ); // Set the full user data upon successful registration
        if (kDebugMode)
          print("AuthService: Registration successful, user set.");
        return {'success': true, 'data': data};
      } else {
        final errorMessage =
            json.decode(response.body)['message'] as String? ??
        ...
        if (kDebugMode)
          print(
            "AuthService: Registration failed - ${response.statusCode}: $errorMessage",
          );
      ...
      if (kDebugMode)
        print("AuthService: Network error during registration - $e");
      clearCurrentUser();
      return {
        'success': false,
        'message': 'Erreur réseau lors de l\'inscription: $e',
      };
    }
  }
}

  ...
  Widget build(BuildContext context) {
    ...
    InputDecoration dec(String label, IconData icon, {bool dense = false}) {
      return InputDecoration(
        ...
        enabledBorder:
            OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.grey.shade300)),
        focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: primary, width: 1.5)),
        filled: true,
        fillColor: card.withAlpha(150),
        contentPadding: EdgeInsets.symmetric(vertical: dense ? 10 : 14, horizontal: 12),
        isDense: dense,
      );
    }
        ...
        child: Form(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            TextFormField(
              ...
              validator: (v) => v == null || v.trim().isEmpty ? 'Email obligatoire' : null,
            ),
            const SizedBox(height: 18),
            ...
            TextFormField(
              ...
              decoration: dec('Description (optionnel)', Icons.description_outlined, dense: true),
              maxLines: 4,
              textCapitalization: TextCapitalization.sentences,
            ),
            const SizedBox(height: 18),
            ...
            GestureDetector(
              ...
              child: AbsorbPointer(
                child: TextFormField(
                  controller: TextEditingController(text: _selectedTypes.join(', ')),
                  decoration: dec(
                    _selectedTypes.isEmpty ? 'Choisir métier(s)' : 'Métier(s) sélectionné(s)',
                    Icons.work_outline,
                  ),
                  validator: (_) => _selectedTypes.isEmpty ? 'Choisissez au moins un métier' : null,
                ),
              ),
            ),
            if (_selectedTypes.isNotEmpty) ...[
              ...
              Wrap(
                spacing: 8,
                runSpacing: 4,
                children: _selectedTypes.map((t) {
                  return Chip(
                    label: Text(t, style: TextStyle(color: primary, fontSize: 12)),
                    backgroundColor: primary.withOpacity(0.15),
                    onDeleted: () => setState(() => _selectedTypes.remove(t)),
                  );
                }).toList(),
              ),
            ],
            ...
            Row(children: [
              Expanded(
                child: TextFormField(
                  ...
                  readOnly: true,
                  decoration: dec('Date de création', Icons.calendar_today_outlined),
              ...
              Expanded(
                child: TextFormField(
                  ...
                  readOnly: true,
                  decoration: dec('Date d\'expiration', Icons.event_busy_outlined),
            ...
            // Images
            Text('Ajouter des Images (max 5)', style: Theme.of(context).textTheme.titleMedium),

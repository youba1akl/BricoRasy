...
class Artisanprofilscreen extends StatefulWidget {
  ...
  final bool
  isMyProfile; // Flag to indicate if it's the logged-in user's profile
...
class _ArtisanprofilscreenState extends State<Artisanprofilscreen> {
  ProfileView _currentView =
      ProfileView.postes; // Default view for content area
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "Impossible de passer l'appel au ${widget.artisan.numTel}",
            ),
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.artisan.image.isNotEmpty) {
      if (widget.artisan.image.startsWith('http')) {
        ...
        print(
          "Artisan image '${widget.artisan.image}' not a URL or full asset path, attempting 'assets/images/${widget.artisan.image}'",
        );
      }
    } else {
      displayImageProvider = const AssetImage(
        'assets/images/defaultprofil.png',
      );
    }
          ...
          if (widget.isMyProfile)
            IconButton(
              ...
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text("Édition du profil artisan à implémenter."),
                  ),
                );
              },
                ...
                children: [
                  CircleAvatar(
                    ...
                    onBackgroundImageError: (exception, stackTrace) {
                      print(
                        "Error loading profile image for ${widget.artisan.fullname}: $exception",
                      );
                    },
                  ),
                        ...
                        children: [
                          Expanded(
                            child: ElevatedButton.icon(
                              ...
                              onPressed:
                                  widget.artisan.numTel.isNotEmpty
                                      ? _handleCall
                                      : null,
                      ...
                      child: ElevatedButton.icon(
                        ...
                        onPressed: () {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                "Navigation vers la gestion des postes à implémenter.",
                              ),
                            ),
                          );
                        },
  ...
  Widget _buildPostesView(
    ...
    return Column(
      ...
      children: [
        Text(
          "Postes de ${widget.artisan.fullname}",
    ...
    return Column(
      ...
      children: [
        ...
        _buildReviewItem(
          context,
          "Utilisateur A",
          "Excellent service, très professionnel et rapide !",
          5,
        ),
        const Divider(),

...
class _ArtisantscreenState extends State<Artisantscreen> {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Échec du chargement des artisans. Veuillez réessayer.'),
  ...
  void _applyAllFilters() {
    ...
    if (_searchQuery.isNotEmpty) {
      ...
      tempList = tempList.where((artisan) {
    ...
    if (_selectedRatingFilter == 'Bien Noté') {
      tempList = tempList.where((artisan) {
        final double ratingValue = double.tryParse(artisan.rating.replaceAll(',', '.')) ?? 0.0;
        return ratingValue >= 3.0; // Your threshold for "Bien Noté"
      }).toList();
    } else if (_selectedRatingFilter == 'Mal Noté') {
      tempList = tempList.where((artisan) {
        final double ratingValue = double.tryParse(artisan.rating.replaceAll(',', '.')) ?? 0.0;
        return ratingValue < 2.0; // Your threshold for "Mal Noté" (e.g., less than 2 stars)
      }).toList();
    }
  ...
  void _handleAdvancedFilterTap() {
    debugPrint('Artisantscreen: Advanced filter icon tapped (implement if needed).');
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _navigateToArtisanProfile(Artisan artisan) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => Artisanprofilscreen(
          artisan: artisan,      // Pass the full Artisan object
          isMyProfile: false,    // When viewing from this list, it's never "my profile"
        ),
      ),
    );
  }
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: ArtisanFilterBar( // This widget should internally have the ['Tout', 'Bien Noté', 'Mal Noté'] filters
            ...
            Expanded(
              child: _isLoading
                  ...
                  : _filteredArtisans.isEmpty
                      ...
                      : ListView.builder(
                          padding: const EdgeInsets.fromLTRB(16, 8, 16, 16), // Adjusted top padding
                          itemCount: _filteredArtisans.length,
                          itemBuilder: (BuildContext context, int index) {
                            final Artisan artisan = _filteredArtisans[index];
                            return Padding(
                              padding: const EdgeInsets.only(bottom: 14.0), // Spacing between items
                              child: ArtisanListItem( // Your custom widget to display each artisan
                                artisan: artisan,
